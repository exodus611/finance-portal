...
(function () {
  ...
  function badgeClass(effort) {
    if (effort === "un document") return "arn-badge arn-badge--easy";
    if (effort === "commission") return "arn-badge arn-badge--hard";
  ...
  function renderCommonRules() {
    ...
    DATA.common_rules.forEach(function (r) {
      ...
      if (r.article && r.article !== "—") {
        var a = el("span", "arn-art", "article " + r.article);
  ...
  function updateCalc() {
    ...
    if (shareNote) {
      if (share < 1) {
        shareNote.textContent = "Le logement dépasse " + limit + " m² : le calcul ne porte donc que sur la part de la facture correspondant aux " + limit + " m².";
  ...
  function updateTimer() {
    ...
    if (!input.value) {
      valEl.textContent = "—";
      if (capEl) capEl.textContent = "Indiquez la date pour voir le temps restant";
    ...
    if (res.state === "expired") {
      valEl.textContent = "Délai dépassé";
      if (capEl) capEl.textContent = "Plus de " + w + " mois se sont écoulés depuis votre enregistrement comme olé. Consultez les autres motifs dans le tableau ci-dessous — rien n'est perdu.";
      if (barEl) barEl.style.width = "100%";
      return;
    }
    if (res.state === "future") {
      valEl.textContent = "—";
      if (capEl) capEl.textContent = "La date indiquée est dans le futur, vérifiez-la.";
      if (barEl) barEl.style.width = "0%";
      return;
    }
    var m = res.windowLeft;
    valEl.textContent = m + " " + pluralRu(m, "mois", "mois", "mois");
    if (capEl) {
      capEl.textContent = " restent pour déposer la demande. La réduction est accordée pendant " + b +
        " " + pluralRu(b, "mois", "mois", "mois") + " sur " + w + ", dont déjà écoulés : " +
        res.passed + " " + pluralRu(res.passed, "mois", "mois", "mois") + ".";
    }
