...
# -*- coding: utf-8 -*-
"""
Вставляет значения из data/avtala.json прямо в avtala.html, между парными
маркерами <!--pr:путь-->значение<!--/pr-->. Нужно, чтобы поисковый робот
видел цифры сразу, не дожидаясь исполнения сценариев.

Замена идёт только по точным маркерам, поэтому повторный запуск даёт
тот же результат и файл не растёт (идемпотентно).
"""
...
HTML = os.path.join(BASE, "avtala.html")
DATA = os.path.join(BASE, "data", "avtala.json")
